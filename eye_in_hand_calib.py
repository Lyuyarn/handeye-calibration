...
def calibrate_eye_in_hand(
    ...
    """
    眼在手上标定（camera mounted on gripper, 求解 cam→gripper）。
    返回：
    - results: [(R_cam2gripper, t_cam2gripper)]，按 method 顺序
    ...
    if len(R_gripper2base) != len(R_board2cam):
        print(
            f"❌ 图像数与机器人位姿数不匹配: "
            f"图像数={len(R_board2cam)}, 机器人位姿数={len(R_gripper2base)}"
        )
    ...
    for method in methods:
        ...
        save_path = src_dir / f"cam_T_gripper_{method}.npy"
...
def validate_AX_XB_eye_in_hand(
    results: List[Tuple[np.ndarray, np.ndarray]],
    ...
    """
    使用 AX = XB 关系做简单验证。
    这里 X 取第一个方法的结果（cam→gripper）。
    """
    if len(R_gripper2base) < 2 or len(R_board2cam) < 2:
        print("样本数量不足，无法进行 AX=XB 验证")
        return

    print("\n===== AX = XB 简单验证（eye-in-hand）=====")

    # 取两组样本
    idx1, idx2 = 0, 1
    Tg1 = HTM(R=R_gripper2base[idx1], P=t_gripper2base[idx1])
    Tg2 = HTM(R=R_gripper2base[idx2], P=t_gripper2base[idx2])
    ...
    X = HTM(R=results[0][0], P=results[0][1]).matrix
...
def validate_constant_base_board(
    src_dir: Path,
    X_cam2gripper: np.ndarray,
    board,
    robot_poses: List[HTM],
) -> None:
    """
    根据待验证的 X = ^{grip}_{cam}T 、相机读出的 ^{cam}_{board}T
    以及机械臂读出的 ^{base}_{grip}T，反推出 ^{base}_{board}T，
    检查是否近似为常量。
    """
    print("\n===== 标定结果验证：^base_board T 是否为常量（eye-in-hand）=====")
    ...
    for i, img in enumerate(img_files):
        ...
        htm_cam2grip = HTM(X_cam2gripper)
        htm_grip2base = robot_poses[i]

        # ^base_board T = ^base_grip T · ^grip_cam T · ^cam_board T
        htm_board2base = htm_grip2base @ htm_cam2grip @ htm_board2cam
        np.set_printoptions(precision=5, suppress=True)
        tmp.append(htm_board2base)
        print(f"反推 base_T_board 标定板到基座结果：\n{htm_board2base}")
...
def main():
    ...
    results, _, _ = calibrate_eye_in_hand(src_dir)

    # AX = XB 验证
    validate_AX_XB_eye_in_hand(
        results, R_gripper2base, t_gripper2base, R_board2cam, t_board2cam
    )

    # 使用第一个方法结果做 base_board 常量验证
    X_cam2gripper = compose_transform(results[0][0], results[0][1])
    validate_constant_base_board(src_dir, X_cam2gripper, board, robot_poses)
