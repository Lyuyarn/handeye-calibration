...
images_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"2025-08-26_2")
...
def func():
    ...
    for i in range(1, len(images_num) + 1):   #标定好的图片在images_path路径下，从0.jpg到x.jpg
        ...
        image_file = os.path.join(images_path,f"{i}.png")
        ...
        if os.path.exists(image_file):
            ...
            if ret:
                ...
                logger_.warning(f'没有找到角点 {image_file}')
                error_num.append(i)
    error_num.append(14)            
    N = len(img_points)
    ...
    poses_main(images_path,error_num)
    # 机器人末端在基座标系下的位姿

    csv_file = os.path.join(path,"RobotToolPoseInHand.csv")
    ...
    for name, method in methods:
        R, t = cv2.calibrateHandEye(R_tool, t_tool, rvecs, tvecs,method)
        results[name] = (R, t)
        print(f"Method {name}: R={R}, t={t}")
    return R,t
